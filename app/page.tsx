...
import {
  Sparkles, Copy, Check, AlertTriangle,
  Stethoscope, FileText, Heart, XCircle, List,
  Brain, Siren, BookOpen, Printer,
  Calculator, CaseUpper, GitBranch,
  Activity, ArrowLeft, HeartPulse, Wind, Biohazard
} from 'lucide-react';
...
const initialTasks: KanbanTask[] = [
  { id: '1', patientName: 'J.S.M.', age: '64a', gender: 'M', complaint: 'Dor Torácica', acuity: 'orange', waitTime: '12min', status: 'exam' },
  { id: '2', patientName: 'M.A.L.', age: '32a', gender: 'F', complaint: 'Cefaleia Súbita', acuity: 'yellow', waitTime: '45min', status: 'wait' },
  { id: '3', patientName: 'R.P.S.', age: '78a', gender: 'M', complaint: 'Dispneia', acuity: 'red', waitTime: '05min', status: 'reval' },
];

// Helper for Library Icons
const getLibraryIcon = (iconName: string) => {
  const map: Record<string, any> = {
    HeartPulse, Wind, Brain, Activity, Siren, Biohazard
  };
...
export default function Home() {
  const [viewMode, setViewMode] = React.useState<'dashboard' | 'selection' | 'protocol' | 'library' | 'accessibility'>('dashboard');
  ...
  const generateNote = React.useCallback(() => {
    ...
    if (patient.medications) {
       patient.medications.forEach(med => {
          patient.allergies.forEach(allergy => {
             if (med.toLowerCase().includes(allergy.toLowerCase()) || allergy.toLowerCase().includes(med.toLowerCase())) {
    ...
    if (alerts.length > 0) {
        blocks.push({ id: 'safety_check', title: 'ALERTAS DE SEGURANÇA', iconName: 'siren', content: alerts.join('\n\n') });
    }
    ...
    blocks.push({
      ...
      iconName: 'alert',
      content: (patient.allergies.length > 0 ? `REFERE ALERGIA A: ${patient.allergies.join(', ')}.` : "Nega alergias.") +
               (patient.medications.length > 0 ? `\nUso: ${patient.medications.join(', ')}.` : "\nNega uso contínuo.")
    });

    // 3. HDA (History of Present Illness) - SYNCHRONIZED
    const protocolName = complaintsData.complaints.find(c => mapComplaintToProtocol(c.id) === activeProtocolId)?.title || "Atendimento Geral";
    ...
    blocks.push({ id: 'exame_fisico', title: 'Exame Físico', iconName: 'stethoscope', content: efFindings.join('. ') || 'Sem particularidades ao exame direcionado.' });

    // 5. Hypotheses
    blocks.push({ id: 'hypotheses', title: 'Hipóteses', iconName: 'brain', content: getHypotheses(activeProtocolId).join('\n') });

    // 6. Conduct
    let conduct = activeRedFlags.length > 0 ? "Protocolo de Alta Complexidade / Emergência." : "Protocolo de Risco Habitual / Sintomáticos.";
    if (alerts.length > 0) conduct += "\n!!! ATENÇÃO AOS ALERTAS DE SEGURANÇA !!!";

    blocks.push({ id: 'conduta', title: 'Conduta', iconName: 'alert', content: conduct, alerts: activeRedFlags.map(f => f.label) });
          ...
          {viewMode === 'dashboard' && (
             <div className="h-full overflow-hidden flex flex-col">
                <DashboardView
                  ...
                  onSettings={() => alert("Painel de configurações em desenvolvimento.")}
                />
             </div>
          )}
                 ...
                 {viewMode === 'library' && (
                     <div className="h-full bg-white/40 dark:bg-slate-800/40 backdrop-blur-3xl backdrop-saturate-[180%] rounded-[36px] border border-white/60 dark:border-white/5 shadow-sm overflow-y-auto custom-scrollbar p-8">
                         <div className="flex items-center gap-3 mb-8">
                            ...
                            <div>
                              <h2 className="text-2xl font-bold text-slate-800 dark:text-white tracking-tight">Biblioteca de Protocolos</h2>
                              <p className="text-sm text-slate-500 dark:text-slate-400">Calculadoras e scores clínicos disponíveis.</p>
                            </div>
                         </div>
                         <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                           {complaintsData.groups.map(group => {
                             ...
                             return (
                               <div key={`lib-${group.code}`} className="bg-white/60 dark:bg-slate-800/60 rounded-[24px] p-6 border border-slate-200/50 dark:border-slate-700/50 shadow-sm flex flex-col gap-4 hover:shadow-md transition-all">
                                  <div className="flex items-center gap-3">
                                     ...
                                     <h3 className="font-bold text-slate-700 dark:text-slate-200">{group.label}</h3>
                                  </div>
                                  <div>
                                    <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Ferramentas</p>
                                    <p className="text-sm font-medium text-slate-600 dark:text-slate-300">{calculator}</p>
                                  </div>
                                  <button
                                    ...
                                    onClick={() => { if(group.code === 'CV') setIsCalculatorOpen(true); }}
                                  >
                         ...
                         <div className="hidden xl:flex w-[400px] shrink-0 flex-col h-full bg-white/70 dark:bg-slate-800/70 backdrop-blur-3xl rounded-[36px] border border-white/60 dark:border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.05)] overflow-hidden transition-all">
                             ...
                             <div className="px-6 py-5 border-b border-slate-100/50 dark:border-slate-700/50 flex justify-between items-center bg-white/30 dark:bg-slate-900/30 shrink-0">
                                <div className="flex items-center gap-2.5">
                                   ...
                                   <h3 className="font-bold text-sm text-slate-800 dark:text-white tracking-wide">Nota Inteligente</h3>
                                </div>
                                <div className="flex gap-1">
                                   <button onClick={() => setIsUppercaseMode(!isUppercaseMode)} className="p-2 hover:bg-black/5 dark:hover:bg-white/10 rounded-xl text-slate-400 transition-colors" title="Maiúsculas"><CaseUpper className="w-4 h-4" /></button>
                                   <button onClick={handlePrint} className="p-2 hover:bg-black/5 dark:hover:bg-white/10 rounded-xl text-slate-400 transition-colors" title="Imprimir"><Printer className="w-4 h-4" /></button>
                                </div>
                             </div>
                             ...
                             <div className="flex-1 p-6 space-y-4 overflow-y-auto custom-scrollbar bg-white/20 dark:bg-black/10">
                                {noteBlocks.map((block) => (
                                   <div key={block.id} className={`group rounded-2xl border p-4 transition-all hover:shadow-lg bg-white/80 dark:bg-slate-800/80 backdrop-blur-md ${block.iconName === 'siren' ? 'border-red-200 dark:border-red-900/50 bg-red-50/80 dark:bg-red-900/10' : 'border-slate-100 dark:border-slate-700/50'}`}>
                                      ...
                                      {block.id === 'sources' ? (
                                         <div className="space-y-2">
                                            {getStructuredReferences(activeProtocolId).map((ref, idx) => (
                                               <div key={idx} className="flex gap-3 items-start p-2 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                                                  <BookOpen className="w-4 h-4 text-blue-500 mt-0.5" />
                                                  <div>
                                                     <p className="text-xs font-bold text-slate-700 dark:text-slate-200">{ref.title}</p>
                                                     <div className="flex gap-2 mt-1">
                                                        <span className="text-[9px] font-bold bg-slate-100 dark:bg-slate-700 px-1.5 rounded text-slate-500">{ref.source}</span>
                                                     </div>
                                                  </div>
                                               </div>
                                            ))}
                                         ...
                                         <textarea
                                            ...
                                            value={block.content}
                                            onChange={(e) => setNoteBlocks(prev => prev.map(b => b.id === block.id ? { ...b, content: e.target.value } : b))}
                                            rows={5}
                                         />
                                      ) : (
                                         <div className={`text-sm leading-relaxed whitespace-pre-wrap ${isUppercaseMode ? 'uppercase' : ''} ${block.iconName === 'siren' ? 'text-red-700 dark:text-red-300 font-bold' : 'text-slate-600 dark:text-slate-300'}`}>
                                            {block.content || <span className="italic text-slate-400">Aguardando dados...</span>}
                                         </div>
                                      )}
                                      ...
                                      {block.alerts && block.alerts.length > 0 && (
                                         <div className="mt-3 flex flex-wrap gap-2 pt-2 border-t border-red-100 dark:border-red-900/30">
                                            {block.alerts.map(a => (
                                               <span key={a} className="inline-flex items-center gap-1 px-2 py-1 rounded-lg bg-red-100 dark:bg-red-900/40 text-[10px] font-bold text-red-600 dark:text-red-300 border border-red-200 dark:border-red-800">
                                ...
                                <div className="relative">
                                   ...
                                   {isFlowMenuOpen && (
                                      <div className="absolute bottom-full left-0 right-0 mb-3 bg-white/90 dark:bg-slate-800/90 backdrop-blur-xl rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-700 p-2 animate-in fade-in slide-in-from-bottom-2 duration-200">
                                         {[
                                            { id: 'wait', label: 'Aguardando Resultado', color: 'text-yellow-600' },
                                            { id: 'exam', label: 'Encaminhar Exames', color: 'text-blue-600' },
                                            { id: 'done', label: 'Alta / Internação', color: 'text-emerald-600' },
                                         ].map(opt => (
                                            <button key={opt.id} onClick={() => handleSetFlow(opt.id as any)} className={`w-full text-left px-4 py-3 rounded-xl text-sm font-bold hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors ${opt.color}`}>
                                               {opt.label}
                                            </button>
                                         ))}
                                      </div>
       ...
       {isAddModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-md p-4 animate-in fade-in duration-200">
             <div className="bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl rounded-[28px] w-full max-w-md p-6 shadow-2xl border border-white/20 dark:border-white/10 ring-1 ring-black/5">
                <h3 className="text-lg font-bold text-slate-800 dark:text-white mb-4">Adicionar Sintoma</h3>
                <input autoFocus type="text" value={newSymptomName} onChange={(e) => setNewSymptomName(e.target.value)} placeholder="Nome do sintoma..." className="w-full px-4 py-3.5 rounded-xl bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 focus:ring-2 focus:ring-blue-500 outline-none mb-6 dark:text-white" onKeyDown={(e) => e.key === 'Enter' && handleSaveCustomSymptom()} />
                <div className="flex gap-3">
                   <button onClick={() => setIsAddModalOpen(false)} className="flex-1 py-3.5 rounded-xl font-bold text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">Cancelar</button>
                   <button onClick={handleSaveCustomSymptom} className="flex-1 py-3.5 rounded-xl font-bold bg-blue-500 text-white hover:bg-blue-600 shadow-lg shadow-blue-500/30 transition-all">Adicionar</button>
                </div>
             </div>
          </div>
       )}

       <HeartScoreCalculator isOpen={isCalculatorOpen} onClose={() => setIsCalculatorOpen(false)} patient={patient} antecedentesItems={sections.find(s => s.id === 'antecedentes')?.items} />
    </div>
  );
}
