...
export const GlassRange: React.FC<GlassRangeProps> = React.memo(({ item, value, onChange }) => {
  ...
  return (
    <div className="px-7 py-8 border-b border-white/4 first:rounded-t-[32px] last:rounded-b-[32px] last:border-0 relative group/range overflow-hidden glass-texture">
       <div className="flex justify-between items-center mb-9 px-1 relative z-10">
         <div className="flex flex-col gap-1">
            <span className="text-[10px] font-extrabold text-slate-400 dark:text-slate-500 uppercase tracking-widest opacity-60 leading-none">
              {item.label}
            </span>
         ...
         <div className="w-14 h-14 rounded-[20px] bg-blue-600/5 dark:bg-blue-400/10 border border-blue-500/10 flex flex-col items-center justify-center transition-all duration-500 group-hover/range:scale-105 rim-highlight">
            <span className="text-xl font-black text-blue-600 dark:text-blue-400 leading-none">{value}</span>
            <span className="text-[8px] font-black text-blue-600/60 dark:text-blue-400/60 uppercase tracking-tighter mt-1">EVA</span>
         </div>
       </div>
