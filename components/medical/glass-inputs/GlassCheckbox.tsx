...
export const GlassCheckbox: React.FC<GlassCheckboxProps> = React.memo(({ item, value, onChange, tooltipText }) => {
  return (
    <motion.button 
      ...
      className={`
        group w-full flex items-center justify-between px-7 py-6 first:rounded-t-[32px] last:rounded-b-[32px] border-b border-white/[0.04] last:border-0 transition-all duration-300 relative overflow-hidden glass-texture
        ${value 
    ...
    >
      <div className="flex items-center gap-6 relative z-10">
        ...
        <div className="flex flex-col items-start gap-0.5">
          <span className={`text-[15px] text-left tracking-tight transition-all duration-500 ${value ? 'text-slate-900 dark:text-white font-[700]' : 'text-slate-500 dark:text-slate-400 font-[500]'}`}>
            {item.label}
          </span>
          ...
          {tooltipText && (
            ...
            >
               <span className="text-[9px] text-blue-500/60 font-[800] uppercase tracking-[0.1em]">Info Adicional</span>
      ...
      <AnimatePresence>
        {item.isRedFlag && value && (
          <motion.div 
            ...
            className="flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-rose-500/10 border border-rose-500/20 text-[9px] font-[800] text-rose-500 uppercase tracking-widest shadow-sm relative z-10"
          >
