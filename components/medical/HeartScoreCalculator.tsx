...
export const HeartScoreCalculator: React.FC<HeartScoreCalculatorProps> = ({
  ...
  const content = (
    <div className={`flex flex-col overflow-hidden ${mode === 'modal' ? 'bg-white dark:bg-slate-900 w-full max-w-2xl rounded-3xl shadow-2xl border border-white/20 dark:border-white/10 max-h-[90vh]' : 'bg-transparent w-full'}`}>
      ...
      <div className={`p-6 overflow-y-auto custom-scrollbar space-y-6 ${mode === 'modal' ? 'bg-white dark:bg-slate-900' : ''}`}>
         {/* H - History */}
      ...
      <div className={`p-6 border-t border-slate-100 dark:border-slate-700 ${risk.bg}`}>
         <div className="flex justify-between items-center">
            ...
            <div className="text-right flex flex-col items-end gap-2">
               <div className={`inline-block px-3 py-1 rounded-lg font-bold text-sm ${risk.color} bg-white/50 dark:bg-slate-900/50 border border-current shadow-sm`}>
                  {risk.label}
               </div>
               {onApply && (
                 <button 
                    onClick={handleApply}
                    className="px-4 py-2 bg-slate-900 dark:bg-white text-white dark:text-black text-xs font-bold uppercase tracking-widest rounded-xl hover:scale-105 active:scale-95 transition-all shadow-lg"
                 >
                   Aplicar na Conduta
                 </button>
               )}
