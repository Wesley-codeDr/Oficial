...
export const PatientContextModal: React.FC<PatientContextModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 overflow-hidden">
      ...
      <motion.div
        role="dialog"
        aria-labelledby="patient-context-modal-title"
        aria-describedby="patient-context-modal-description"
        aria-modal="true"
      ...
      >
        ...
        <div className={cn(
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2
                id="patient-context-modal-title"
                className={cn(
              ...
              <p
                id="patient-context-modal-description"
                className={cn(
              ...
              {showAdvanced && (
                ...
                >
                  <section className="space-y-2">
                    ...
                    <div className="flex gap-2" role="group" aria-label="Selecionar tipo de evolução">
                      {(['agudo', 'subagudo', 'crônico'] as const).map((type) => (
                        <button
                          key={type}
                          onClick={() => setEvolutionType(evolutionType === type ? undefined : type)}
                          aria-pressed={evolutionType === type}
                          className={cn(
                  ...
                  <section className="space-y-2">
                    ...
                    <div className="flex gap-2" role="group" aria-label="Selecionar tipo de início">
                      {(['súbito', 'progressivo', 'insidioso'] as const).map((type) => (
                        <button
                          key={type}
                          onClick={() => setOnsetType(onsetType === type ? undefined : type)}
                          aria-pressed={onsetType === type}
                          className={cn(
                            'flex-1 py-3 rounded-xl text-[10px] font-black uppercase border transition-all',
                            onsetType === type
                              ? 'bg-blue-500 text-white border-transparent'
