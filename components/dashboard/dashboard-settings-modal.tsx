      ...
      <div className="w-full max-w-2xl bg-white/90 dark:bg-[#1c1c1e]/90 backdrop-blur-2xl rounded-[32px] shadow-2xl border border-white/20 dark:border-white/10 overflow-hidden flex flex-col max-h-[85vh]">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-8">
                ...
                return (
                  ...
                  >
                    ...
                    <button
                      ...
                      className={`p-2 rounded-xl transition-colors ${isVisible ? 'text-ios-blue bg-blue-50 dark:bg-blue-900/20' : 'text-slate-400'}`}
                    >
          ...
          <section className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4 flex items-center gap-2">
                <Rows className="w-3.5 h-3.5" /> Secões
              </h3>
              <div className="space-y-3">
                ...
                <label className="flex items-center justify-between p-3 rounded-2xl bg-white/40 dark:bg-white/5 border border-slate-200 dark:border-white/5 cursor-pointer">
                  ...
                  <div
                    className={`w-11 h-6 rounded-full p-1 transition-colors duration-300 ${preferences.showGreeting ? 'bg-ios-green' : 'bg-slate-300 dark:bg-slate-600'}`}
                    onClick={(e) => {
                      e.preventDefault();
                  ...
                  >
                    <div
                      className={`w-4 h-4 bg-white rounded-full shadow-sm transform transition-transform duration-300 ${preferences.showGreeting ? 'translate-x-5' : 'translate-x-0'}`}
                    />
                  </div>
                </label>
                ...
                <label className="flex items-center justify-between p-3 rounded-2xl bg-white/40 dark:bg-white/5 border border-slate-200 dark:border-white/5 cursor-pointer">
                  ...
                  <div
                    className={`w-11 h-6 rounded-full p-1 transition-colors duration-300 ${preferences.showAlertRow ? 'bg-ios-green' : 'bg-slate-300 dark:bg-slate-600'}`}
                    onClick={(e) => {
                      e.preventDefault();
                  ...
                  >
                    <div
                      className={`w-4 h-4 bg-white rounded-full shadow-sm transform transition-transform duration-300 ${preferences.showAlertRow ? 'translate-x-5' : 'translate-x-0'}`}
                    />
                  </div>
                </label>
              </div>
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-400 mt-2.5 px-1 leading-relaxed">
                O modo compacto reduz espaçamentos verticais para exibir mais informacões em telas
                menores.
              </p>
            </div>
          </section>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {Object.entries(COLUMN_LABELS).map(([id, label]) => {
                ...
                return (
                  ...
                  >
                    <span
                      className={`text-sm font-semibold ${isVisible ? 'text-blue-700 dark:text-blue-300' : 'text-slate-600 dark:text-slate-400'}`}
                    >
                      {label}
                    </span>
                    <div
                      className={`
                        ...
                        ${isVisible ? 'bg-ios-blue text-white' : 'bg-slate-200 dark:bg-slate-700 text-transparent'}
                      `}
                    >
